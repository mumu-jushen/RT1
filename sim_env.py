...
# def compute_ik(robot_id, end_effector_index, target_pos, target_ori=None):
#     """
#     计算机器人逆运动学，获取关节角度。

#     Parameters:
#     - robot_id (int): 机器人ID。
#     - end_effector_index (int): 末端执行器的链接索引。
#     - target_pos (list): 目标位置 [x, y, z]。
#     - target_ori (list, optional): 目标姿态四元数 [x, y, z, w]。

#     Returns:
#     - joint_positions (list): 逆运动学求解得到的关节角度。
#     """
...
# def control_joints(robot_id, joint_indices, joint_positions):
#     """
#     控制机器人的关节位置。

#     Parameters:
#     - robot_id (int): 机器人ID。
#     - joint_indices (list): 需要控制的关节索引列表。
#     - joint_positions (list): 对应的关节角度列表。
#     """
...
#             force=500  # 您可以根据需要调整这个值
#         )

# # 首先选择你想要运行的任务环境
# task_name = "touch"  # 可以选择 "touch" 或 "pick"

# # 初始化 PyBullet 的仿真环境
# p.connect(p.GUI)
# # 设置物理引擎参数，不启用锥形摩擦
# p.setPhysicsEngineParameter(enableConeFriction=0)
# p.setAdditionalSearchPath('d:\\Anaconda3\\envs\\igibson\\lib\\site-packages\\pybullet_data')
...
#     # 4. 随机重置目标物体的位置和朝向
#     target_object_range = [0.3, 0.7, -0.2, 0.2, -np.pi, np.pi]  # [x_min, x_max, y_min, y_max, rot_min, rot_max]
#     env.reset_tar_obj(tar_obj_range=target_object_range, random_pos_rot=True)

#     # 5. 获取机器人的末端执行器索引（根据您的机器人定义，可能需要调整）
#     ee_index = env.robot.ee_index

#     # 6. 定义抓取的位置（与目标物体的位置相同）
#     target_pos, target_ori = p.getBasePositionAndOrientation(env.tar_obj)
#     target_ori = p.getQuaternionFromEuler([0, -np.pi, 0])

#     grasp_pos = list(target_pos)
#     grasp_pos[2] += 0.1  # 在物体上方 10cm

#     # 7. 计算逆运动学，获取机器人的关节角度
#     joint_positions = compute_ik(env.robot.robot_id, ee_index, grasp_pos, target_ori)

#     # 8. 设置机器人的关节位置
#     num_joints = len(joint_positions)
#     joint_indices = [i for i in range(num_joints)]
#     control_joints(env.robot.robot_id, joint_indices, joint_positions)

#     # 9. 运行仿真循环并保存每帧图像和末端位姿
#     for i in range(24):  # 1 秒钟的仿真（240 个时间步）
#         p.stepSimulation()
#         time.sleep(1./24.)

#         # 捕获当前视角的图像
#         img = get_cam_view_img('front', env.robot.robot_id, ee_index)
...
#         # 获取末端执行器的位姿
#         ee_pos, ee_ori = p.getLinkState(env.robot.robot_id, ee_index)[0:2]
...
#     # 10. 控制机器人降低末端执行器，准备抓取
#     grasp_pos[2] -= 0.1  # 下降 10cm，接近物体
#     joint_positions = compute_ik(env.robot.robot_id, ee_index, grasp_pos, target_ori)
#     control_joints(env.robot.robot_id, joint_indices, joint_positions)
...
#         # 保存图像到文件夹
#         img_file_path = os.path.join(output_dir, f"iteration_{iteration}_frame_{i+120}.png")
#         Image.fromarray(img).save(img_file_path)
...
#     # 11. 关闭夹爪（假设您的 Panda 机器人有夹爪控制）
#     env.robot.move_gripper(0)
...
#     # 12. 提升物体
#     grasp_pos[2] += 0.2  # 上升 20cm
#     joint_positions = compute_ik(env.robot.robot_id, ee_index, grasp_pos, target_ori)
#     control_joints(env.robot.robot_id, joint_indices, joint_positions)

#     for i in range(24):  # 提升需要更多时间
#         p.stepSimulation()
#         time.sleep(1./24.)
...
#         # 保存图像到文件夹
#         img_file_path = os.path.join(output_dir, f"iteration_{iteration}_frame_{i+360}.png")
#         Image.fromarray(img).save(img_file_path)
...
# # 将记录的位姿数据转换为DataFrame并保存到Excel文件中
